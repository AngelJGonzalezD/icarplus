...
if ($_REQUEST['action'] == "ajax" && isset($_REQUEST['id'])) {
    $id = $_REQUEST['id'];

    // Realiza una consulta a la base de datos para obtener la información del vehículo por ID
    $query = "SELECT * FROM vehiculo WHERE id = $id";
    // Realiza la consulta y obtén el resultado, asumiendo que estás utilizando MySQLi
    ...
    // Termina el script para evitar que se ejecute el resto del código
    exit();
}
